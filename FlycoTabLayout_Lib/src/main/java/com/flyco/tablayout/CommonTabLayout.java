...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
  ...
  private void updateTabSelection(int position) {
    for (int i = 0; i < mTabCount; ++i) {
      ...
      CustomTabEntity tabEntity = mTabEntitys.get(i);
      iv_tab_icon.setImageResource(
          isSelect ? tabEntity.getTabSelectedIcon() : tabEntity.getTabUnselectedIcon());
  ...
  public void setMsgMargin(int position, float leftPadding, float bottomPadding) {
    ...
    if (tipView != null) {
      ...
      if (mIconGravity == Gravity.TOP || mIconGravity == Gravity.BOTTOM) {
        ...
        lp.topMargin =
            mHeight > 0 ? (int) (mHeight - textHeight - iconH - margin) / 2 - dp2px(bottomPadding)
                : dp2px(bottomPadding);
      } else {
        lp.leftMargin = dp2px(leftPadding);
        lp.topMargin =
            mHeight > 0 ? (int) (mHeight - Math.max(textHeight, iconH)) / 2 - dp2px(bottomPadding)
                : dp2px(bottomPadding);
      }
